...
The public contract is now stable. This release follows three consecutive clean
cross-vendor certification rounds over the same functional surface
(`d91c025..12394b4`), plus a clean current-surface soak of both `--gate-with` and
`--max-permission`. The version/docs edits in this release are metadata only; no
certified runtime behavior changed after the three-round surface froze.

### Certification

| Round | Reviewer | Focus | Result |
|---|---|---|---|
| 1 | `quick-reviewer` / Claude Sonnet 5 | Full feedback and section-3 surface | **CLEAN** — minor/nit only |
| 2 | fresh `quick-reviewer` / Claude Sonnet 5 | Security, concurrency, negative paths | **CLEAN** — minor only |
| 3 | fresh `quick-reviewer` / Claude Sonnet 5 | Windows/POSIX, public contracts, test strength | **CLEAN** — minor only |

Every counting envelope reported `status:success`, `execution_status:success`,
`report_ok:true`, normalized `verdict:pass`, a served Claude model, and no suspect,
repair, or caller-resume signal. Failed, timed-out, and malformed-report attempts
were explicitly excluded rather than converted into clean credit.

The separate control soak used a real gate and permission ceiling on exact HEAD
`12394b4`: the gate explicitly approved, the one-attempt reviewer returned CLEAN,
and no repository change occurred.

### Stability promise

- CLI flags/subcommands and their documented semantics are public API.
- Envelope schema remains independently versioned as `envelope:1`; breaking shape
  changes require an envelope bump and breaking public-contract changes require 2.0.
- Agent frontmatter, report fields, and documented exit codes are stable surfaces.
- Additive fields and backward-compatible behavior may ship in 1.x minors; fixes
  that preserve the contract may ship in patches.
...
making promises a backend will not keep. Along the way it fixes the error message most
likely to have wasted your afternoon.

### Breaking

- **`agy` at `read-only` is now refused.** agy cannot enforce that tier and never could:
  `--sandbox` restricts terminal operations only, `--mode plan` does not withhold the file
  tools, and withholding the workspace only breaks *relative* paths -- a declared read-only
  agy agent read a secret file and created another by absolute path. summon fails closed
  rather than name a boundary that does not exist.
  Use a backend that enforces the tier (claude, codex, cursor-agent), declare `safe-edit` if
  write access is acceptable, or set `SUMMON_ALLOW_UNENFORCED_READONLY=1` to proceed with
  the tier marked **advisory** in `warnings`. The opt-in waives a tier *you* declared -- it
  can never waive one summon imposed for a `--gate-with` adjudicator, a `--max-permission`
  clamp, or a corrective resume. `--dry-run` reports `would_refuse` before you spend
  anything.

### Added

- **Argv preflight with an honest error.** Every CLI backend receives the prompt through
  `argv`, and Windows caps the whole command line at 32767 characters -- then reports the
  overflow as a *missing file*. summon relayed that as `CLI not found`, which is almost
  certainly behind the standing reports that "the codex CLI isn't installed". summon now
  measures the real serialized line (UTF-16 units on Windows, encoded bytes and `ARG_MAX`
  on POSIX, including your environment) and refuses before spawning with a message naming
  argv as the cause. `--prompt-file` does not avoid the limit; write large material to a
  file under `--cwd` and ask the agent to read it.
- **Cross-request protection for shared result paths.** An envelope whose `request_sha256`
  does not match the job being run is refused with `result_path_conflict` instead of served
  as an answer. Two manifest runs sharing a `--results-dir` previously had one parent
  reporting the other's result, with both exiting success.
- **`exit_history` and full exit telemetry on repaired envelopes.** When a corrective resume
  supersedes an earlier attempt, every superseded attempt is kept in order and the published
  exit tuple describes the run that actually produced the answer.
- **`doctor --probe` discloses which tier it exercised** (`probed_permission`), in both JSON
  and human output, and probes agy inside a throwaway directory rather than your repo.

### Fixed

- **`--gate-with` now covers every execution path**, including the contract-repair resume --
  and the gate adjudicates *the request about to run* rather than the caller's original one.
  A gate authorises one execution; an approval for a request nobody executes is worse than
  no gate, because it looks like coverage.
- **An agent definition can no longer widen its own permissions.** Frontmatter `args:` are
  appended after the permission flags, so `args: ["--add-dir", "/repo"]` silently rewrote
  the tier summon had just computed. Boundary-moving flags are stripped, in both the
  double-dash and Go-style single-dash spellings agy accepts.
- **Build-time failures return an envelope, not a traceback.** summon's contract is one JSON
  object on stdout, always.
- **Archive claiming under contention** now measures directory writability directly instead
  of inferring it from a denial counter, so transient races no longer abort a clear and a
  genuinely unwritable directory is diagnosed as such in ~130 probes instead of 10001.
- The manifest's forensics fallback writes atomically like every other envelope path; a
  concurrent reader could previously observe a half-written file.

### Documentation

- Corrected two claims that measurement disproved: agy read-only "cannot read your
  repository", and a shared `--results-dir` being a "wasteful duplicate, not corruption".
  Both now state what the repro showed.
- README documents the argv limit, the agy tier behaviour, and the situational envelope
  fields. `docs/VERSIONING_AND_1.0_CRITERIA.md` records why summon remains 0.x.

### Under the hood

388 dispatcher tests (up from 344) and 22 install tests, on Windows and Ubuntu. Every fix in
this release was mutation-tested -- reverted to its exact broken form to prove the new test
fails -- and the suite now detects tests that leak patched globals, refuses to run with a
test defined where the runner cannot collect it, and asserts the docs still agree with the
code on the claims listed above.

*This release was built through ten rounds of cross-vendor adversarial review (codex
adjudicating Claude-written code), each round re-reviewing the previous round's fixes. The
round-by-round record, including four changelog claims that had to be corrected for
overstating what shipped, is in `docs/VERSIONING_AND_1.0_CRITERIA.md` as the evidence for
why the 1.0 bar is not yet met.*
