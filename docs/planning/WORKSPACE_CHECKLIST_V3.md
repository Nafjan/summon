# Summon workspace implementation checklist — v3

Status: reviewed planning baseline. Three rounds completed; all seven closing-round reports found no material checklist gaps. Every item remains unchecked implementation/verification work. No release approval. See WORKSPACE_REVIEW_LOG.md for dispositions and evidence limits.
...
- [ ] R01 Establish one operation-specific capability registry, with explicit producer/consumer migration to a versioned v2 schema. Preserve the exact resume-capabilities/v1 validator and legacy projections until callers migrate. Key by backend/transport/adapter/version/operation; discovery is candidate evidence, not qualification. Test v1/v2 rejection, unsupported operations and revocation at submission. At every submission, compare the locally observed adapter/executable version and registry generation with qualified evidence; revalidate after queued waits. A version change/revocation causes typed refusal before the next contact. No scheduled live re-probe or auth repair occurs without its own grant; interrupted in-flight handling remains ownership-scoped.
...
- [ ] R14 Extend existing checksummed generation journals with size-triggered checkpoint/rollover and explicit hot/cold/total-retention budgets; segmentation alone does not reduce the aggregate cap. Preserve deduplication and audit facts. Corrupt non-tail authority data blocks execution; verified-prefix inspection is advisory. Test rollover, compaction, torn tails, corrupt middle records and bounded replay without silently skipping evidence. Commit checkpoint/segment membership through an atomic durable manifest or equivalent linearizable scheme before retiring segments. Reuse existing atomic-write/sharing-violation handling. Inject crashes, reader overlap and replacement denial at each commit boundary; replay one committed generation without duplication, or refuse if durability cannot be established.
...
- [ ] F02 Keep roster seat, worker instance, provider session and human principal distinct. Bind workers to project, task scope, epoch and effective authority. Define authenticated local IPC bootstrap and scoped per-worker capability grants bound to task/instance/epoch and permitted operations, delivered over an authenticated owned channel rather than public argv/URLs. Specify supervisor-to-worker authentication, secret storage, rotation, expiry and revocation. Self-declared names or bare PIDs never authenticate a principal. State the trusted OS-user and untrusted worker threat model; same-user ACLs alone do not isolate hostile workers, so enforce verified worker scope or refuse that qualification.
- [ ] F03 Assign one authoritative writer/store to each entity; build workspace indexes as rebuildable projections over existing journals. No second scheduler or dual authority log. Reuse the coordinator's short serialized critical sections, keeping worker claims separate from writer locks. Define CLI/UI/supervisor command arbitration, authenticated admission and bounded contention outcomes; no lock stealing or unsynchronized multi-process append. Concurrent send/cancel tests must preserve the live owner and record order.
...
- [ ] F06 Add stable message/content IDs, correlation/reply/causation IDs, per-stream sequence, sender epoch and recipient binding. Addressing is instance-bound by default; logical-task continuation requires explicit grant and auditable rebinding. No universal wall-clock order or silent successor retargeting. A new Attempt need not create a new WorkerInstance. Logical-task addressing may follow an explicitly authorized successor under its recorded rebinding grant; a crash does not itself authorize replacement, spend or inbox transfer.
- [ ] F07 Define a versioned delivery transition table with accepted, queued, included_in_attempt, submission_started, submitted, acknowledged, not_submitted, held_for_recovery, rejected, expired, cancelled and dead_lettered. Separate logical messages from per-attempt deliveries and adapter acknowledgement level. Exercise every legal/illegal transition; the normative table below governs failure and re-entry. Represent collision_hold as held_for_recovery with typed reason native_turn_collision, not an undeclared new state; recipient_drift is another reason. Contact certainty is orthogonal to the hold reason.
...
- [ ] F15 Coordinate bounded message bytes/count/rate, context capacity, execution slots and provider-contact budgets through the existing owner/admission boundary; do not equate an inbox message with a billable execution slot. Reserve control capacity. Whole-message selection preserves order; oversize head messages get explicit hold/refusal or authorized artifact handling, not silent starvation or truncation. Default to ordered delivery within each declared recipient stream: a held required predecessor visibly pauses that stream. Independently declared streams may progress within unchanged grants; never infer independence from message prose or silently skip a causal predecessor.
- [ ] F16 Define task and attempt state machines independently, including partial, blocked, failed, cancelled and indeterminate. No aggregate task success while required attempts/cleanup remain unresolved. An execution run may close as failed/blocked with unstarted dependencies explicitly unresolved; only aggregate success requires all required work resolved. Terminal run closure cannot erase unknown attempt/spend/cleanup state or rewrite blocked task history. A later authorized graph continuation gets a new execution identity.
- [ ] F17 Provide provider-inert CLI inspection and task/message API projections. Link legacy IDs rather than rewriting old records. Provide authenticated, idempotent message-send and delivery-disposition commands alongside inspection: retain hold, cancel eligible unsubmitted context, dead-letter with reason, or propose a separately linked delivery under a fresh grant. Define CLI/API schema together, with noninteractive refusals and stale-generation tests; do not require raw journal edits.
- [ ] F18 Build a two-locally-authenticated simulated-worker vertical fixture covering recipient inclusion/exclusion, resumed new messages, restart, duplicate delivery, sender/recipient drift, capacity, cancelled queues, authority injection and crashes around submission. This qualifies protocol behavior only; real provider messaging requires L10 evidence. Include identity-driven involuntary fork with queued/included/submitted parent deliveries, one explicit disposition each, and no duplicate child inclusion; include queued recipient replacement and native-turn collision hold validation. Include forged identity, supervisor impersonation, stale/replayed capability and rotation/revocation cases with typed refusal; no fixture may authenticate by trusting a supplied worker name.
...
- [ ] H09 Classify resources as verified Summon-owned children versus externally owned sessions. Birth-token/process-group/Job Object teardown applies only to owned children. External timeout/cancel invokes a qualified scoped host operation or holds/detaches with uncertainty; never OS-kill the external app. Require physical predecessor fencing before successor mutation; worktree separation alone does not stop spend or shared side effects. Persist a conflict fence over affected resources/effect scope while an external predecessor with mutation authority remains unconfirmed, including after detach/expiry. New conflicting mutation stays refused until authoritative quiescence/cancellation or a verified scope fence; manual assertions and a separate worktree alone cannot waive unknown spend/shared effects. Read-only attachments do not imply a blanket workspace write lock.
...
- [ ] H11 Create one idempotent successor only from eligible authenticated state and a newly recorded explicit grant bound to its request, route, permission and contact/spend ceilings. No age-threshold waiver or ambient account/PAYG fallback. Remaining task budgets do not reset; expansion requires a separate typed owner decision. Unknown prior contact/cleanup blocks automatic retry. Keep the full authorized per-attempt reservation held for unknown contact until authoritative reconciliation; no TTL-based refund. Admit successors only against unreserved task capacity. Explicit budget expansion cannot waive cleanup/physical fencing. If a monetary ceiling is not enforceable on a route, label that limitation and refuse a hard-capped launch rather than treating an estimate as a guarantee.
...
- [ ] S02 Gate claims on dependency success and accepted artifact revision. Separate reversible dependency_pending/blocked_by_dependency from terminal task refusal. Reconcile readiness deterministically from immutable graph and upstream evidence after explicit upstream retry; never reopen already completed downstream effects silently. Define deterministic aggregate closure after permanent upstream failure/cancellation: mark the blocked prerequisite and close the execution run without waiting forever. Preserve downstream not-started evidence; automatic terminal cascade is allowed only under an explicitly declared graph policy and cannot reopen prior effects.
...
- [ ] S08 Run sixteen fake workers with dependencies, competing claims, lease expiry, duplicate completion, cancellation, host restart and stale artifacts. Measure coordinator separately from provider children. Include at least one v1-pinned worker and verify typed graph-version refusal rather than silently granting a claim.
...
- [ ] X02 SessionDescriptor binds opaque private handle and generation, provider/host/adapter version, workspace/worktree scope, principal/account/model evidence, granted operations, reachability boundary, expiry and per-operation payload transport. Public projections omit handles, private root/account fingerprints and raw paths. Store private executable/session capabilities in an authenticated access-restricted sidecar separate from ordinary context/export projections, with scope-bound opaque references. Test tampering, missing sidecars, permission failure and exclusion from compiled agent context. An ordinary journal is not inherently public.
...
- [ ] X14 Provide copy/export manual handoff when no adapter is available. Require explicit content selection and redacted preview; never silently send via UI automation. Record a manual export as a separate Handoff artifact/event, not provider submission or acknowledgement. Preserve pending delivery disposition explicitly. Optional pasted replies are human-supplied unverified external context linked by correlation; they cannot impersonate an authenticated worker, certify a model or become a ballot.
...
- [ ] U01 Redesign atlas as one task-first navigation shell over scoped projections of existing kernels. Choose routes for task timeline/review/decision views with independent typed authorization; do not merge authority tokens, use raw cross-origin iframe tokens or expose session capabilities in history/links. Use a single local facade for scoped workspace projections with server-side typed-kernel dispatch and authorization on every operation; avoid a generic bearer-token-forwarding proxy or raw iframe integration. Specify origin/CSRF protection, session bootstrap, scope separation and secret-safe navigation in the implementation design, retaining legacy surfaces during migration.
...
- [ ] U04 Specify per-message boundary badges, reason and timestamps for queued-next-turn, included, submitted, held, rejected and cancelled states. Keep owner/stream/turn/model evidence separate. Never infer acknowledgement or exact per-message token cost from generic completion. Map every normative delivery state, including submission_started, not_submitted, acknowledged, expired and dead_lettered. Show not_submitted as proven no contact, never as automatically retrying; display separate contact/spend certainty and next permitted action.
...
- [ ] U13 Add an existing-session connection journey showing supported operations and delivery evidence, plus an honest manual handoff fallback. Surface native_turn_collision as an accessible nonintrusive hold with retain/cancel-eligible/manual-handoff actions and an explicit next-turn reconciliation path. Export never marks a message delivered; expiration cannot erase uncertainty. Noninteractive callers receive a typed result rather than a blocking prompt.
- [ ] U14 Keep on-demand CLI mode daemonless: messages remain durable and delivery/readiness are evaluated on the next explicitly authorized invocation. An explicitly started foreground/background supervisor may drive approved queued work; record its owner/lifetime and display passive versus supervised mode. No hidden daemon startup or automatic lease retry. Show passive mode persistently and explain which explicit action evaluates a queued message. Offer only implemented copyable commands or authorized UI controls; starting supervision is a distinct action. Display owner state without exposing raw process/account/session details.
...
- [ ] L07 Keep receipt/evidence storage outside public source. Treat hashes of low-entropy private values as potentially identifying; public projections must be purpose-limited. Before storing private workspace context, define and test OS-user-scoped access controls for store creation/reopen, inheritance, reparse paths and exports. Refuse unenforceable declared protection. Document that ACLs are access control, not encryption or protection from an administrator/the trusted OS user. Optional encryption and broader multi-user support remain separate scope decisions.
...
- [ ] L09 Extend the fixed gate registry with behavioral and rendered evidence, retaining existing HTTP/security and source checks under accurately described scope. Record browser/engine/toolchain versions. Require machine-readable skip reasons and a reviewed allowlist; a required rendered gate cannot pass because all cases skipped. Version any gate-contract change explicitly. Inventory and update all gate-name/schema consumers together, including tools/release_manifest.py, tools/release_gates.py, their tests and the CI expected_gates equality assertion. A versioned contract change must provide old/new evidence fixtures and a deterministic stale-consumer refusal; do not merely add a manifest entry.
...
- [ ] D06 Adopt a release support/deprecation policy and explicit storage/privacy retention rules before workspace GA. The support/deprecation policy also applies to the first preview minor: explicitly approve and announce any R02 resume demotion, affected lanes, typed reason and non-launching fork path before it ships.


## Normative release cut and execution modes

The next-minor candidate is a **protocol/workspace preview**, not native-session or long-horizon GA. Its minimum vertical slice is R01-R04, R09-R13, R15-R16, F01-F18, a minimal U01/U02/U03/U04/U07/U08/U14 shell and G01-G08 compatibility/non-escalation checks plus L01-L09/L11-L12 release gates. L10 is mandatory for each real provider/adapter claim and cannot be replaced by F18. Existing-regression items R05-R08 must pass for any affected shipping route. E03/E04/E06/E08/E12 authority, context and roster invariants apply to the slice. R14's capacity design must be explicit before the preview admits work; full size-triggered archival implementation and S-scale qualification may follow under strict existing bounds. U07 accessibility applies to the shipped UI, not just a later redesign. All implementation owners/estimates are assigned before development; role owners above are responsibility functions.
...
| Preview release | R01-R04, R09-R13, R15-R16, F01-F18, U01-U04, U07-U08, U14, G01-G08, L01-L09, L11-L12; affected R05-R08; R14 capacity design; E03/E04/E06/E08/E12; L10 per live claim | Committed-source-bound manifest, privacy/migration/skip inventory, owner approval |

## Normative delivery recovery table

Logical Message IDs remain immutable. Each inclusion/submission is a separate Delivery bound to an Attempt. Acknowledgement level and evidence are fields, not inference from task success. Manual handoff has its own artifact/event lifecycle and is never a provider-delivery acknowledgement. A scalar cursor is insufficient when messages are held/deferred: persist explicit sets or a cursor plus authenticated holes.
...
| queued/included_in_attempt/submission_started/submitted → held_for_recovery | Typed native_turn_collision, recipient_drift, grant_revoked, contact/cleanup uncertainty or incompatible fork; retain original binding and refuse automatic resubmission |
...
| held_for_recovery/dead_lettered → separately linked delivery | Explicit disposition plus a fresh attempt grant and physical fence; prior delivery history stays immutable |
