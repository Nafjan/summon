...
.turn-recovery-actions .restore { border-color: var(--blue); color: var(--blue); }
  ...
  <main class="thread" aria-labelledby="title">
    ...
    <div class="thread-body">
      ...
      <div class="activity-dock" aria-label="Active provider work"><div id="working" class="working" aria-live="off"></div><section id="turn-recovery" class="turn-recovery" hidden aria-live="polite" aria-labelledby="turn-recovery-title">
        ...
        <div class="turn-recovery-actions"><button id="turn-recover" class="recover" type="button">Mark blocked and close</button><button id="turn-restore" class="restore" type="button" hidden>Restore submitted draft</button><button id="turn-fork" class="fork" type="button">Start a fresh conversation</button></div>
      </section></div>
...
_JS = r'''(() => {
  ...
  const state = {
    ...
    unread: {}, lastRoomIndex: '', drafts: {}, pendingDrafts: {}, lastUpdated: null, streamLive: false, connectionKind: 'offline', feedPinned: true, railOpen: false, evidenceOpen: false, reviewing: false, recovery: null, refusal: null,
  ...
  async function getJSON(url, options = {}) {
    ...
    if (!response.ok) { const error = new Error(data.error || data.error_kind || 'request refused'); error.status = response.status; error.data = data; throw error; }
    return data;
  }
  ...
  function modelProof(payload, participant) {
    ...
    if (payload && typeof payload.refusal_reason === 'string' && payload.refusal_reason) return {label: 'CONTINUATION REFUSED · ' + payload.refusal_reason, className: 'drift', caption: 'No provider dispatch occurred for this continuation; choose an explicit fresh conversation or restore the submitted draft.', title: 'Continuation was refused before dispatcher launch; provider contact was not recorded.'};
  ...
  function pendingDraftKey(participant, turnId, sessionId = state.selected) { return [String(sessionId || ''), String(participant || ''), String(turnId || '')].join('|'); }
  function recoveryCandidate() {
    for (let index = state.events.length - 1; index >= 0; index -= 1) {
      ...
      const refusal = typeof payload.refusal_reason === 'string';
      if (['blocked', 'error', 'timeout'].includes(status) || drift || refusal) return {record, participant, status, drift, refusal};
      return null;
    }
    return null;
  }
  function renderRecovery() {
    const candidate = state.refusal || recoveryCandidate(); state.recovery = candidate; const panel = $('turn-recovery'); if (!candidate) { panel.hidden = true; return; }
    panel.hidden = false; const item = identity(candidate.participant); const payload = candidate.record && candidate.record.payload ? candidate.record.payload : {};
    const pendingKey = candidate.refusal && candidate.record ? pendingDraftKey(candidate.participant, payload.turn_id) : null; const pending = pendingKey ? state.pendingDrafts[pendingKey] : null;
    if (pending && !$('message').value.trim()) { $('message').value = pending; delete state.pendingDrafts[pendingKey]; saveDraft(); updateCounter(); }
    const refusalText = candidate.refusal ? 'Continuation was refused before dispatcher launch. No provider contact was recorded; start a fresh conversation explicitly.' : null;
    $('turn-recovery-copy').replaceChildren(text(refusalText || (candidate.drift ? 'The provider reported a different model than the roster expected. Start a fresh conversation instead of silently continuing.' : 'The last agent turn did not close cleanly. Choose how to close it; Summon will not retry the provider automatically.')));
    $('turn-recovery-facts').replaceChildren(text(identityLabel(candidate.participant) + ' · ' + (candidate.refusal ? String(payload.refusal_reason || candidate.reason || 'continuation refused') : (candidate.status || 'model mismatch')) + ' · event ' + String(candidate.record ? candidate.record.cursor || '—' : 'not recorded')));
    $('turn-recover').hidden = Boolean(candidate.drift || candidate.refusal); $('turn-fork').hidden = false; $('turn-restore').hidden = !(candidate.refusal && pending && $('message').value.trim() && $('message').value !== pending);
    $('turn-recover').setAttribute('aria-label', 'Mark ' + item.name + ' turn blocked'); $('turn-fork').setAttribute('aria-label', 'Start a fresh conversation from ' + item.name + ' turn'); $('turn-restore').setAttribute('aria-label', 'Restore the submitted draft for ' + item.name);
  }
  function renderRoom(data) {
    const previousRoom = state.room; const previousParticipant = $('participant').value;
    state.refusal = null; state.room = data.room || {}; state.events = Array.isArray(data.events) ? data.events.slice() : []; state.cursor = Number(data.cursor || state.events.length || 0); state.streamCursor = state.cursor; state.unread[state.selected] = false; $('latest').hidden = true; markUpdated(); rebuildActive(); updateHeader(); loadDraft();
  ...
  function appendRecord(record) {
    ...
    const anchor = captureFeedAnchor(); state.events.push(record); state.cursor = cursor; state.streamCursor = cursor; markUpdated(); rebuildActive(); updateHeader(); renderRecovery(); renderWorking(); updateComposer();
  ...
  function updateComposer() {
    ...
    $('caption').replaceChildren(text(context ? 'Saved as room context. It cannot approve or change a run. Ctrl/Cmd+Enter posts it.' : 'Review before launch · ' + target + (item && item.provider ? ' via ' + item.provider : '') + ' · output is context, not approval. A refusal or model mismatch appears in the recovery controls. Ctrl/Cmd+Enter opens review.')); $('send').classList.toggle('agent-action', !context); $('send').replaceChildren(text(context ? 'Post context' : (state.reviewing ? 'Review open' : 'Review agent request')));
  ...
  function updateCounter() { $('counter').replaceChildren(text(String($('message').value.length) + ' / 12000')); }
  ...
  async function launchTurn() { const participant = $('participant').value; const value = $('message').value; const requestSession = state.selected; const activeKey = key(requestSession, participant); if (!requestSession || !participant || !value.trim() || state.active[activeKey]) return; state.reviewing = false; state.active[activeKey] = {participant, turn_id: null, startedAt: Date.now()}; renderWorking(); updateComposer(); $('note').className = 'composer-note'; $('note').replaceChildren(text('Request recorded; waiting for ' + identityLabel(participant) + '…')); try { const result = await getJSON('/api/v1/rooms/' + encodeURIComponent(requestSession) + '/turns', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({participant, message: value, reviewed: true})}); const turnId = String(result.turn_id || ''); const current = state.active[activeKey]; if (current) current.turn_id = turnId || null; if (turnId) state.pendingDrafts[pendingDraftKey(participant, turnId, requestSession)] = value; if (state.selected === requestSession && $('message').value === value) { $('message').value = ''; saveDraft(); $('note').replaceChildren(text('Agent turn accepted; waiting for recorded completion.')); } if (state.selected === requestSession) await syncRoom(); } catch (error) { if (state.selected === requestSession) { delete state.active[activeKey]; if (error && error.data && error.data.error_kind === 'chat_resume_refused') { state.refusal = {participant, status: 'blocked', refusal: true, reason: error.data.refusal && error.data.refusal.reason_code, record: null}; if (!$('message').value.trim() || $('message').value === value) { $('message').value = value; saveDraft(); } renderRecovery(); $('note').className = 'composer-note error'; $('note').replaceChildren(text('Continuation refused before dispatch; no provider contact was recorded. Choose an explicit fresh conversation.')); } else { $('note').className = 'composer-note error'; $('note').replaceChildren(text(publicError(error, 'Agent turn could not start; the recorded timeline was left unchanged.'))); } renderWorking(); } } finally { if (state.selected === requestSession) { updateComposer(); $('message').focus(); } } }
  ...
  function restoreSubmittedDraft() { const candidate = state.recovery; if (!candidate || !candidate.record || !candidate.refusal) return; const payload = candidate.record.payload || {}; const draftKey = pendingDraftKey(candidate.participant, payload.turn_id, state.selected); const draft = state.pendingDrafts[draftKey]; if (!draft) return; $('message').value = draft; delete state.pendingDrafts[draftKey]; saveDraft(); updateCounter(); updateComposer(); renderRecovery(); $('note').className = 'composer-note'; $('note').replaceChildren(text('Submitted draft restored; review it before sending again.')); $('message').focus(); }
  function chooseMode(mode) { state.mode = mode; state.reviewing = false; updateComposer(); $('message').focus(); }
  $('context-tab').addEventListener('click', () => chooseMode('context')); $('agent-tab').addEventListener('click', () => chooseMode('agent')); $('turn-recover').addEventListener('click', recoverTurn); $('turn-restore').addEventListener('click', restoreSubmittedDraft); $('turn-fork').addEventListener('click', forkTurn);
