...
(() => {{
  ...
  function render(data) {{
    ...
    evidenceList.replaceChildren(); const plans=receipt.plan_identity_by_seat||{{}}; const planLabels=Object.keys(plans).map(seat => {{ const plan=plans[seat]||{{}}; return seat+': '+(plan.cli||plan.transport||'bound'); }}).join(' · '); const durable=receipt.durable_context||null; const contextState=durable ? String(durable.state||'stale_refused').replaceAll('_',' ') + (Array.isArray(durable.mismatch_reasons)&&durable.mismatch_reasons.length ? ' · '+durable.mismatch_reasons.join(', ') : '') : 'Not supplied'; const contextFacts=durable ? 'age '+String(durable.actual_age_ms||0)+' ms · revision delta '+String(durable.actual_revision_delta||0)+' · affected '+String(durable.affected_entry_count||0) : '—'; const lines=[['Run',data.run_id],['Decision',receipt.decision_id],['Context freshness',contextState],['Context evidence',contextFacts],['Seats',(receipt.seat_ids||[]).join(', ')],['Agents',planLabels||'Bound seat plans'],['Roster',receipt.roster_digest ? String(receipt.roster_digest).slice(0,12)+'…' : 'Bound to receipt'],['Cleanup',cleanup ? (cleanup.clean === true && cleanup.verified === true ? 'Verified clean' : 'Needs review') : 'Pending'],['Recovery',data.recovery_required ? 'Required' : 'Clear'],['Deadline',receipt.deadline_unix_ms ? new Date(receipt.deadline_unix_ms).toLocaleTimeString([],{{hour:'2-digit',minute:'2-digit'}}) : 'Bound to receipt']]; lines.forEach(([label,value]) => {{ const p=document.createElement('p'); const strong=document.createElement('strong'); strong.append(text(label+': ')); p.append(strong,text(value == null || value === '' ? '—' : value)); evidenceList.append(p); }});
