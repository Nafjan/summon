...
rem Tell the Python entry point which transport was used so it can refuse every
rem raw --prompt value. Use --prompt-file for every dispatch prompt;
rem batch expansion can otherwise truncate or reinterpret it before Python sees argv.
