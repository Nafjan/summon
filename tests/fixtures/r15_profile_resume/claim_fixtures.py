# Adapted public fixture preparation for real selected-profile resolution and owned
# executable observation; authenticated source/qualification/claim writers are real.
from dataclasses import replace
from types import SimpleNamespace
import hashlib, json, os
from pathlib import Path
import _profiles, _executor, _launch_binding
...
def _eligible_source(tmp_path, *, commands=(), allow_text_only=False,
    ...
    profile_dir.mkdir()
    (profile_dir / '.credentials.json').write_text('{"fixture":"before"}', encoding='utf-8')
    registry=tmp_path/'profiles.json'
    registry.write_text(json.dumps({'profiles':{'review-profile':{'cli':'claude','config_dir':str(profile_dir),'auth_mode':'login'}}}),encoding='utf-8')
    os.environ['SUMMON_PROFILES_FILE']=str(registry)
    selected=_profiles.resolve_profile('review-profile','claude',invocation.cwd)
    invocation = replace(
        invocation, profile="review-profile", profile_auth_mode='login',
    ...
    result["profile"] = {
        ...
        "registry_sha256": selected["registry_sha256"], "command_sha256": None, "auth_mode":"login",
    }
    result['resume']['session_id']='12345678-1234-1234-1234-123456789abc'
    identity=_executor.build_request_identity(agent='reviewer',prompt='private',cwd=invocation.cwd,agents_dir=str(tmp_path/'agents'),cli='claude',model='claude-opus-5',profile='review-profile')
    result['request_sha256']=_executor.request_fingerprint(**identity)
    executable=tmp_path/'claude.exe'
    executable.write_bytes(b'owned synthetic executable identity; never executed')
    result["_private_launch_observation"] = _launch_binding.observation(str(executable),[],invocation.cwd,selected['env'],backend='claude',external_cli_version='claude-test/1.0')
...
def _prepare(tmp_path, *, gate_with=None, legacy_profile_binding=False):
    root, job_id = _eligible_source(tmp_path)
    if legacy_profile_binding:
        source_path = Path(continuation.continuation_path(root, job_id))
        source = continuation._read_strict(source_path)
        source["backend"].pop("profile_path", None)
        source["backend"].pop("profile_state_sha256", None)
        source["schema"] = continuation.SCHEMA
        body = {key: value for key, value in source.items() if key != "auth"}
        source["auth"] = continuation._auth("nonce", body)
        _jobs._atomic_write_json(source_path, source)
