...
| **Native fan-out** | `--manifest jobs.json --concurrency agy=2,codex=3` runs a whole swarm: per-backend throttles, one atomic envelope per job, **skip-if-done resume** (re-run a crashed batch, only the missing jobs re-dispatch). Or `--background`+`--worktree` for hand-scheduled waves. |
| **Structured output** | `--json-schema verdict.json` extracts the agent's final JSON, validates it, and retries once on mismatch — `parsed`/`parse_ok` instead of a brittle outermost-braces heuristic. |
| **Verifiable, not vibes** | The envelope echoes `model.resolved` (what the backend *actually served* — catches a typo'd model silently falling back to default) and the exact `permission_flags` a run used. `--dry-run` shows the whole resolved dispatch without spending a token. |
